...
class HandRecog:
    ...
    def __init__(self, hand_label):
        ...
        self.fist_start_time = None  # Initialize fist_start_time
        self.last_gesture_time = 0   # Initialize last_gesture_time
    ...
    def set_finger_state(self):
        ...
        # Set thumb state
        if thumb_angle > 30:  # Thumb is clearly up
            self.finger = self.finger | 16
        
        # Set other finger states
    ...
    def get_gesture(self):
        ...
        # Add cooldown check
        if current_time - self.last_gesture_time < 0.5:  # 0.5 second cooldown
            return self.prev_gesture
            
        current_gesture = Gest.PALM
        
        # First priority: Check for fist/drag - all fingers down including thumb
        if self.finger == 0:  # Binary 00000 - all fingers down
            if self.fist_start_time is None:
                self.fist_start_time = current_time
            elif current_time - self.fist_start_time > 1.0:  # 1 second hold
                current_gesture = Gest.FIST
        else:
            self.fist_start_time = None
            
        # Second priority: Check for right click - ONLY index finger up
        if self.finger == 8 and current_gesture == Gest.PALM:  # Binary 1000 - only index finger
            current_gesture = Gest.INDEX
            
        # Third priority: Check for pinch gestures
        if current_gesture == Gest.PALM and self.finger in [Gest.LAST3, Gest.LAST4] and self.get_dist([8,4]) < 0.05:
            if self.hand_label == HLabel.MINOR:
                current_gesture = Gest.PINCH_MINOR
            else:
                current_gesture = Gest.PINCH_MAJOR
        
        # Fourth priority: Check for V gesture and two finger closed
        elif current_gesture == Gest.PALM and self.finger == Gest.FIRST2:
            point = [[8,12],[5,9]]
...
class Controller:
    ...
    def changesystembrightness():
        ...
        currentBrightnessLv = sbcontrol.get_brightness(display=0)[0]/100.0  # Get first value from list
    ...
    def handle_controls(gesture, hand_result):  
        ...
        if gesture == Gest.V_GEST:
            ...
            # Right click only when V gesture was shown first (flag is True)
        ...
        elif gesture == Gest.INDEX and not Controller.flag:
            # Scroll only when V gesture was not shown first (flag is False)
            y_pos = hand_result.landmark[8].y  # Index fingertip
            y_base = hand_result.landmark[5].y  # Index base
            scroll_amount = int((y_base - y_pos) * 300)  # Scale factor for smoother scrolling
...
class GestureController:
    ...
    def __init__(self):
        ...
        print("[DEBUG] Initializing GestureController and opening camera...")
        GestureController.gc_mode = 1
        GestureController.cap = cv2.VideoCapture(0)
        if not GestureController.cap.isOpened():
            print("[ERROR] Could not open camera. Please check if the camera is connected or used by another application.")
...
def main():
    print("[INFO] Testing OpenCV window...")
    test_img = np.zeros((300, 400, 3), dtype=np.uint8)
    cv2.putText(test_img, "OpenCV Test Window", (30, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    cv2.imshow("OpenCV Test", test_img)
    print("[INFO] Waiting for key press in OpenCV window...")
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    print("[INFO] Test complete. Now running gesture control...")
    gc = None
    try:
        gc = GestureController()
        gc.start()
    except Exception as e:
        print(f"[ERROR] Error in gesture control: {str(e)}")
        import traceback
        traceback.print_exc()
        input("\nPress Enter to close this window...")
    finally:
        if gc is not None and gc.cap is not None:
            gc.cap.release()
        cv2.destroyAllWindows()
